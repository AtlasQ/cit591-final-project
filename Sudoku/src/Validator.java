...
public class Validator {
    ...
    // This is to method to tell if user input right number, this is comparing
    // user's input to answer
    ...
    public boolean isValid(int n, int rowNum, int colNum, NumberTable nt) {
        ...
        // now square
        // we divide the table into 9 chunks, first let's find out which row chunk are
        // we in
        // for example, rowNum = 2, then colChunk = 0, which means the number is in the
        // top row chunk
        int rowChunk = rowNum / 3;
        // for example, if colNum = 8, then colChunk = 2, which means the number is in
        // the right column chunk
        int colChunk = colNum / 3;

        // chunk number time 3 will be the the start of the index, time 3 plus 2 will be
